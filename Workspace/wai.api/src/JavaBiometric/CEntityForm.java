...
import JavaBiometric.Biometrics.CFingerPrint;
import JavaBiometric.Biometrics.CFingerPrintGraphics;
...
public class CEntityForm extends JFrame {
  ...
  public CEntityForm(String path1, String path2) {
   jButtonStep1.addActionListener(new java.awt.event.ActionListener() {
    ...
    //picture1
    //Set picture new
    //m_bimage1=ImageIO.read(new File(new java.io.File("").getAbsolutePath()+path1));
  ...
  //  m_bimage2=ImageIO.read(new File(new java.io.File("").getAbsolutePath()+"\\ProcessedSample2.bmp")) ;
    //m_bimage2=ImageIO.read(new File(new java.io.File("").getAbsolutePath()+path2)) ;
  ...
  private void jButtonStep3_actionPerformed(ActionEvent e)
  {
     ...
     try
      {
        System.out.println("Match%="+m_finger1.Match(finger1 , finger2,65,false));
      }
      catch (Exception ex)
      {
